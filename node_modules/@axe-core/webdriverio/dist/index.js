...
var isWebdriverClient = (client) => {
  ...
  const c = client;
  if (typeof c.switchToFrame !== "function" && typeof c.switchFrame !== "function") {
    return false;
  }
  return true;
};
...
async function clientSwitchFrame(client, id) {
  const c = client;
  if (typeof c.switchFrame === "function") {
    return await c.switchFrame(
      id
    );
  } else {
...
async function clientSwitchWindow(client, handle) {
  const c = client;
...
async function assertFrameReady(client) {
  try {
    ...
    const isBiDi = typeof client.switchFrame === "function";
    const executePromise = isBiDi ? client.execute(() => {
      return document.readyState === "complete" && document.URL !== "about:blank";
...
var AxeBuilder = class {
   ...
   */
  async inject(browsingContext = null, browsingContextId = null) {
 ...
 Please check out ${this.errorUrl}`
   ...
   */
  async runPartialRecursive(context, frameStack = [], topWindow) {
    if (topWindow === void 0) {
