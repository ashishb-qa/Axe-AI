...
exports.evaluateFormLabels = exports.evaluateLinkClarityBatch = exports.evaluateLinkClarity = exports.evaluateAltTextMeaningfulness = void 0;
const axios_1 = __importDefault(require("axios"));
const runtime_config_js_1 = require("../config/runtime.config.js");
const clamp = (value) => Math.max(0, Math.min(1, value));
async function callLlm(request) {
    if (runtime_config_js_1.runtimeConfig.ai.provider === 'heuristic' || !runtime_config_js_1.runtimeConfig.ai.endpoint || !runtime_config_js_1.runtimeConfig.ai.apiKey) {
        return null;
    }
    const prompt = [
    ...
    const response = await axios_1.default.post(runtime_config_js_1.runtimeConfig.ai.endpoint, {
        model: runtime_config_js_1.runtimeConfig.ai.model,
        messages: [{ role: 'user', content: prompt }],
        temperature: 0
    }, {
        headers: {
            Authorization: `Bearer ${runtime_config_js_1.runtimeConfig.ai.apiKey}`,
...
function heuristicAltText(imageContext, altText) {
...
function heuristicLinkClarity(text) {
...
function heuristicFormLabels(labels) {
    if (labels.length === 0) {
        return {
            type: 'form_labels',
            score: 0,
...
async function evaluateAltTextMeaningfulness(imageContext, altText) {
...
exports.evaluateAltTextMeaningfulness = evaluateAltTextMeaningfulness;
async function evaluateLinkClarity(text) {
    const llm = await callLlm({ type: 'link_clarity', text });
    return llm ?? heuristicLinkClarity(text);
}
exports.evaluateLinkClarity = evaluateLinkClarity;
async function evaluateLinkClarityBatch(texts) {
    if (texts.length === 0)
        return [];
    ...
    if (runtime_config_js_1.runtimeConfig.ai.provider !== 'heuristic' && runtime_config_js_1.runtimeConfig.ai.endpoint && runtime_config_js_1.runtimeConfig.ai.apiKey) {
        ...
        try {
            const response = await axios_1.default.post(runtime_config_js_1.runtimeConfig.ai.endpoint, { model: runtime_config_js_1.runtimeConfig.ai.model, messages: [{ role: 'user', content: prompt }], temperature: 0 }, { headers: { Authorization: `Bearer ${runtime_config_js_1.runtimeConfig.ai.apiKey}`, 'Content-Type': 'application/json' }, timeout: 30000 });
            const raw = response.data?.choices?.[0]?.message?.content ?? '';
            const content = raw.replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '').trim();
            const parsed = JSON.parse(content);
            resultMap = new Map(unique.map((text, i) => [
                text,
                {
                    type: 'link_clarity',
...
exports.evaluateLinkClarityBatch = evaluateLinkClarityBatch;
async function evaluateFormLabels(labels) {
    const llm = await callLlm({ type: 'form_labels', labels });
    return llm ?? heuristicFormLabels(labels);
}
exports.evaluateFormLabels = evaluateFormLabels;
