...
export async function writeJsonReport(report: ComplianceReport, path = 'reports/rgaa-compliance-report.json') {
  await mkdir(dirname(path), { recursive: true });
  await writeFile(path, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
}

export async function writeHtmlReport(report: ComplianceReport, path = 'reports/rgaa-compliance-report.html') {
